...
if (mysql_num_rows( mysql_query("SHOW TABLES LIKE '".settings."'"))) {
	...
	$sql = "INSERT INTO currencies
			(conversion, name, currency, symbol, insert_time) VALUES 
			(0, 'Albania Lek', 'ALL', 'Lek', '" . $current_timestamp . "'),
			(0, 'Afghanistan Afghani', 'AFN', '؋', '" . $current_timestamp . "'),
			(0, 'Argentina Peso', 'ARS', '$', '" . $current_timestamp . "'),
			(0, 'Aruba Guilder', 'AWG', 'ƒ', '" . $current_timestamp . "'),
			(0, 'Australia Dollar', 'AUD', '$', '" . $current_timestamp . "'),
			(0, 'Azerbaijan New Manat', 'AZN', '" . 'ман' . "', '" . $current_timestamp . "'),
			(0, 'Bahamas Dollar', 'BSD', '$', '" . $current_timestamp . "'),
			(0, 'Barbados Dollar', 'BBD', '$', '" . $current_timestamp . "'),
			(0, 'Belarus Ruble', 'BYR', 'p.', '" . $current_timestamp . "'),
			(0, 'Belize Dollar', 'BZD', 'BZ$', '" . $current_timestamp . "'),
			(0, 'Bermuda Dollar', 'BMD', '$', '" . $current_timestamp . "'),
			(0, 'Bolivia Boliviano', 'BOB', '\$b', '" . $current_timestamp . "'),
			(0, 'Bosnia and Herzegovina Convertible Marka', 'BAM', 'KM', '" . $current_timestamp . "'),
			(0, 'Botswana Pula', 'BWP', 'P', '" . $current_timestamp . "'),
			(0, 'Bulgaria Lev', 'BGN', 'лв', '" . $current_timestamp . "'),
			(0, 'Brazil Real', 'BRL', 'R$', '" . $current_timestamp . "'),
			(0, 'Brunei Darussalam Dollar', 'BND', '$', '" . $current_timestamp . "'),
			(0, 'Cambodia Riel', 'KHR', '៛', '" . $current_timestamp . "'),
			(1, 'Canada Dollar', 'CAD', '$', '" . $current_timestamp . "'),
			(0, 'Cayman Islands Dollar', 'KYD', '$', '" . $current_timestamp . "'),
			(0, 'Chile Peso', 'CLP', '$', '" . $current_timestamp . "'),
			(0, 'China Yuan Renminbi', 'CNY', '¥', '" . $current_timestamp . "'),
			(0, 'Colombia Peso', 'COP', '$', '" . $current_timestamp . "'),
			(0, 'Costa Rica Colon', 'CRC', '₡', '" . $current_timestamp . "'),
			(0, 'Croatia Kuna', 'HRK', 'kn', '" . $current_timestamp . "'),
			(0, 'Cuba Peso', 'CUP', '₱', '" . $current_timestamp . "'),
			(0, 'Czech Republic Koruna', 'CZK', 'Kč', '" . $current_timestamp . "'),
			(0, 'Denmark Krone', 'DKK', 'kr', '" . $current_timestamp . "'),
			(0, 'Dominican Republic Peso', 'DOP', 'RD$', '" . $current_timestamp . "'),
			(0, 'East Caribbean Dollar', 'XCD', '$', '" . $current_timestamp . "'),
			(0, 'Egypt Pound', 'EGP', '£', '" . $current_timestamp . "'),
			(0, 'El Salvador Colon', 'SVC', '$', '" . $current_timestamp . "'),
			(0, 'Estonia Kroon', 'EEK', 'kr', '" . $current_timestamp . "'),
			(0, 'Euro Member Countries', 'EUR', '€', '" . $current_timestamp . "'),
			(0, 'Falkland Islands (Malvinas) Pound', 'FKP', '£', '" . $current_timestamp . "'),
			(0, 'Fiji Dollar', 'FJD', '$', '" . $current_timestamp . "'),
			(0, 'Ghana Cedis', 'GHC', '¢', '" . $current_timestamp . "'),
			(0, 'Gibraltar Pound', 'GIP', '£', '" . $current_timestamp . "'),
			(0, 'Guatemala Quetzal', 'GTQ', 'Q', '" . $current_timestamp . "'),
			(0, 'Guernsey Pound', 'GGP', '£', '" . $current_timestamp . "'),
			(0, 'Guyana Dollar', 'GYD', '$', '" . $current_timestamp . "'),
			(0, 'Honduras Lempira', 'HNL', 'L', '" . $current_timestamp . "'),
			(0, 'Hong Kong Dollar', 'HKD', '$', '" . $current_timestamp . "'),
			(0, 'Hungary Forint', 'HUF', 'Ft', '" . $current_timestamp . "'),
			(0, 'Iceland Krona', 'ISK', 'kr', '" . $current_timestamp . "'),
			(0, 'India Rupee', 'INR', 'Rs', '" . $current_timestamp . "'),
			(0, 'Indonesia Rupiah', 'IDR', 'Rp', '" . $current_timestamp . "'),
			(0, 'Iran Rial', 'IRR', '﷼', '" . $current_timestamp . "'),
			(0, 'Isle of Man Pound', 'IMP', '£', '" . $current_timestamp . "'),
			(0, 'Israel Shekel', 'ILS', '₪', '" . $current_timestamp . "'),
			(0, 'Jamaica Dollar', 'JMD', 'J$', '" . $current_timestamp . "'),
			(0, 'Japan Yen', 'JPY', '¥', '" . $current_timestamp . "'),
			(0, 'Jersey Pound', 'JEP', '£', '" . $current_timestamp . "'),
			(0, 'Kazakhstan Tenge', 'KZT', 'лв', '" . $current_timestamp . "'),
			(0, 'Korea (North) Won', 'KPW', '₩', '" . $current_timestamp . "'),
			(0, 'Korea (South) Won', 'KRW', '₩', '" . $current_timestamp . "'),
			(0, 'Kyrgyzstan Som', 'KGS', 'лв', '" . $current_timestamp . "'),
			(0, 'Laos Kip', 'LAK', '₭', '" . $current_timestamp . "'),
			(0, 'Latvia Lat', 'LVL', 'Ls', '" . $current_timestamp . "'),
			(0, 'Lebanon Pound', 'LBP', '£', '" . $current_timestamp . "'),
			(0, 'Liberia Dollar', 'LRD', '$', '" . $current_timestamp . "'),
			(0, 'Lithuania Litas', 'LTL', 'Lt', '" . $current_timestamp . "'),
			(0, 'Macedonia Denar', 'MKD', 'ден', '" . $current_timestamp . "'),
			(0, 'Malaysia Ringgit', 'RM', 'RM', '" . $current_timestamp . "'),
			(0, 'Mauritius Rupee', 'MUR', '₨', '" . $current_timestamp . "'),
			(0, 'Mexico Peso', 'MXN', '$', '" . $current_timestamp . "'),
			(0, 'Mongolia Tughrik', 'MNT', '₮', '" . $current_timestamp . "'),
			(0, 'Mozambique Metical', 'MZN', 'MT', '" . $current_timestamp . "'),
			(0, 'Namibia Dollar', 'NAD', '$', '" . $current_timestamp . "'),
			(0, 'Nepal Rupee', 'NPR', '₨', '" . $current_timestamp . "'),
			(0, 'Netherlands Antilles Guilder', 'ANG', 'ƒ', '" . $current_timestamp . "'),
			(0, 'New Zealand Dollar', 'NZD', '$', '" . $current_timestamp . "'),
			(0, 'Nicaragua Cordoba', 'NIO', 'C$', '" . $current_timestamp . "'),
			(0, 'Nigeria Naira', 'NGN', '₦', '" . $current_timestamp . "'),
			(0, 'Norway Krone', 'NOK', 'kr', '" . $current_timestamp . "'),
			(0, 'Oman Rial', 'OMR', '﷼', '" . $current_timestamp . "'),
			(0, 'Pakistan Rupee', 'PKR', '₨', '" . $current_timestamp . "'),
			(0, 'Panama Balboa', 'PAB', 'B/.', '" . $current_timestamp . "'),
			(0, 'Paraguay Guarani', 'PYG', 'Gs', '" . $current_timestamp . "'),
			(0, 'Peru Nuevo Sol', 'PEN', 'S/.', '" . $current_timestamp . "'),
			(0, 'Philippines Peso', 'PHP', '₱', '" . $current_timestamp . "'),
			(0, 'Poland Zloty', 'PLN', 'zł', '" . $current_timestamp . "'),
			(0, 'Qatar Riyal', 'QAR', '﷼', '" . $current_timestamp . "'),
			(0, 'Romania New Leu', 'RON', 'lei', '" . $current_timestamp . "'),
			(0, 'Russia Ruble', 'RUB', 'руб', '" . $current_timestamp . "'),
			(0, 'Saint Helena Pound', 'SHP', '£', '" . $current_timestamp . "'),
			(0, 'Saudi Arabia Riyal', 'SAR', '﷼', '" . $current_timestamp . "'),
			(0, 'Serbia Dinar', 'RSD', 'Дин.', '" . $current_timestamp . "'),
			(0, 'Seychelles Rupee', 'SCR', '₨', '" . $current_timestamp . "'),
			(0, 'Singapore Dollar', 'SGD', '$', '" . $current_timestamp . "'),
			(0, 'Solomon Islands Dollar', 'SBD', '$', '" . $current_timestamp . "'),
			(0, 'Somalia Shilling', 'SOS', 'S', '" . $current_timestamp . "'),
			(0, 'South Africa Rand', 'ZAR', 'R', '" . $current_timestamp . "'),
			(0, 'Sri Lanka Rupee', 'LKR', '₨', '" . $current_timestamp . "'),
			(0, 'Sweden Krona', 'SEK', 'kr', '" . $current_timestamp . "'),
			(0, 'Switzerland Franc', 'CHF', 'CHF', '" . $current_timestamp . "'),
			(0, 'Suriname Dollar', 'SRD', '$', '" . $current_timestamp . "'),
			(0, 'Syria Pound', 'SYP', '£', '" . $current_timestamp . "'),
			(0, 'Taiwan New Dollar', 'TWD', 'NT$', '" . $current_timestamp . "'),
			(0, 'Thailand Baht', 'THB', '฿', '" . $current_timestamp . "'),
			(0, 'Trinidad and Tobago Dollar', 'TTD', 'TT$', '" . $current_timestamp . "'),
			(0, 'Turkey Lira', 'TRY', '₤', '" . $current_timestamp . "'),
			(0, 'Tuvalu Dollar', 'TVD', '$', '" . $current_timestamp . "'),
			(0, 'Ukraine Hryvna', 'UAH', '₴', '" . $current_timestamp . "'),
			(0, 'United Kingdom Pound', 'GBP', '£', '" . $current_timestamp . "'),
			(0, 'United States Dollar', 'USD', '$', '" . $current_timestamp . "'),
			(0, 'Uruguay Peso', 'UYU', '\$U', '" . $current_timestamp . "'),
			(0, 'Uzbekistan Som', 'UZS', 'лв', '" . $current_timestamp . "'),
			(0, 'Venezuela Bolivar', 'VEF', 'Bs', '" . $current_timestamp . "'),
			(0, 'Viet Nam Dong', 'VND', '₫', '" . $current_timestamp . "'),
			(0, 'Yemen Rial', 'YER', '﷼', '" . $current_timestamp . "'),
			(0, 'Zimbabwe Dollar', 'ZWD', 'Z$', '" . $current_timestamp . "'),
			(0, 'Emirati Dirham', 'AED', 'د.إ', '" . $current_timestamp . "'),
			(0, 'Malaysian Ringgit', 'MYR', 'RM', '" . $current_timestamp . "'),
			(0, 'Kuwaiti Dinar', 'KWD', 'ك', '" . $current_timestamp . "'),
			(0, 'Moroccan Dirham', 'MAD', 'م.', '" . $current_timestamp . "'),
			(0, 'Iraqi Dinar', 'IQD', 'د.ع', '" . $current_timestamp . "'),
			(0, 'Bangladeshi Taka', 'BDT', 'Tk', '" . $current_timestamp . "'),
			(0, 'Bahraini Dinar', 'BHD', 'BD', '" . $current_timestamp . "'),
			(0, 'Kenyan Shilling', 'KES', 'KSh', '" . $current_timestamp . "'),
			(0, 'CFA Franc', 'XOF', 'CFA', '" . $current_timestamp . "'),
			(0, 'Jordanian Dinar', 'JOD', 'JD', '" . $current_timestamp . "'),
			(0, 'Tunisian Dinar', 'TND', 'د.ت', '" . $current_timestamp . "'),
			(0, 'Ghanaian Cedi', 'GHS', 'GH¢', '" . $current_timestamp . "'),
			(0, 'Central African CFA Franc BEAC', 'XAF', 'FCFA', '" . $current_timestamp . "'),
			(0, 'Algerian Dinar', 'DZD', 'دج', '" . $current_timestamp . "'),
			(0, 'CFP Franc', 'XPF', 'F', '" . $current_timestamp . "'),
			(0, 'Ugandan Shilling', 'UGX', 'USh', '" . $current_timestamp . "'),
			(0, 'Tanzanian Shilling', 'TZS', 'TZS', '" . $current_timestamp . "'),
			(0, 'Ethiopian Birr', 'ETB', 'Br', '" . $current_timestamp . "'),
			(0, 'Georgian Lari', 'GEL', 'GEL', '" . $current_timestamp . "'),
			(0, 'Cuban Convertible Peso', 'CUC', 'CUC$', '" . $current_timestamp . "'),
			(0, 'Burmese Kyat', 'MMK', 'K', '" . $current_timestamp . "'),
			(0, 'Libyan Dinar', 'LYD', 'LD', '" . $current_timestamp . "'),
			(0, 'Zambian Kwacha', 'ZMK', 'ZK', '" . $current_timestamp . "'),
			(0, 'Zambian Kwacha', 'ZMW', 'ZK', '" . $current_timestamp . "'),
			(0, 'Macau Pataca', 'MOP', 'MOP$', '" . $current_timestamp . "'),
			(0, 'Armenian Dram', 'AMD', 'AMD', '" . $current_timestamp . "'),
			(0, 'Angolan Kwanza', 'AOA', 'Kz', '" . $current_timestamp . "'),
			(0, 'Papua New Guinean Kina', 'PGK', 'K', '" . $current_timestamp . "'),
			(0, 'Malagasy Ariary', 'MGA', 'Ar', '" . $current_timestamp . "'),
			(0, 'Ni-Vanuatu Vatu', 'VUV', 'VT', '" . $current_timestamp . "'),
			(0, 'Sudanese Pound', 'SDG', 'SDG', '" . $current_timestamp . "'),
			(0, 'Malawian Kwacha', 'MWK', 'MK', '" . $current_timestamp . "'),
			(0, 'Rwandan Franc', 'RWF', 'FRw', '" . $current_timestamp . "'),
			(0, 'Gambian Dalasi', 'GMD', 'D', '" . $current_timestamp . "'),
			(0, 'Maldivian Rufiyaa', 'MVR', 'Rf', '" . $current_timestamp . "'),
			(0, 'Congolese Franc', 'CDF', 'FC', '" . $current_timestamp . "'),
			(0, 'Djiboutian Franc', 'DJF', 'Fdj', '" . $current_timestamp . "'),
			(0, 'Haitian Gourde', 'HTG', 'G', '" . $current_timestamp . "'),
			(0, 'Samoan Tala', 'WST', '$', '" . $current_timestamp . "'),
			(0, 'Guinean Franc', 'GNF', 'FG', '" . $current_timestamp . "'),
			(0, 'Cape Verdean Escudo', 'CVE', '$', '" . $current_timestamp . "'),
			(0, 'Tongan Pa\'anga', 'TOP', 'T$', '" . $current_timestamp . "'),
			(0, 'Moldovan Leu', 'MDL', 'MDL', '" . $current_timestamp . "'),
			(0, 'Sierra Leonean Leone', 'SLL', 'Le', '" . $current_timestamp . "'),
			(0, 'Burundian Franc', 'BIF', 'FBu', '" . $current_timestamp . "'),
			(0, 'Mauritanian Ouguiya', 'MRO', 'UM', '" . $current_timestamp . "'),
			(0, 'Bhutanese Ngultrum', 'BTN', 'Nu.', '" . $current_timestamp . "'),
			(0, 'Swazi Lilangeni', 'SZL', 'SZL', '" . $current_timestamp . "'),
			(0, 'Tajikistani Somoni', 'TJS', 'TJS', '" . $current_timestamp . "'),
			(0, 'Turkmenistani Manat', 'TMT', 'm', '" . $current_timestamp . "'),
			(0, 'Basotho Loti', 'LSL', 'LSL', '" . $current_timestamp . "'),
			(0, 'Comoran Franc', 'KMF', 'CF', '" . $current_timestamp . "'),
			(0, 'Sao Tomean Dobra', 'STD', 'STD', '" . $current_timestamp . "'),
			(0, 'Seborgan Luigino', 'SPL', 'SPL', '" . $current_timestamp . "')";
	$result = mysql_query($sql,$connection);
