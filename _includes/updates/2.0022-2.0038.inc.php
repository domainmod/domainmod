    ...
    while ($row = mysqli_fetch_object($result)) {
        ...
        if ($row->status != "" || $row->status_notes != "" || $row->notes != "") {
            ...
            $sql_update = "UPDATE domains
                           SET notes = '" . trim(mysqli_real_escape_string($dbcon, $new_notes)) . "',
                                  notes_fixed_temp = '1',
                               update_time = '" . $time->stamp() . "'
                           WHERE id = '" . $row->id . "'";
...
if ($current_db_version === '2.0035') {
    ...
    while ($row = mysqli_fetch_object($result)) {
        ...
        $result_find_new = mysqli_query($dbcon, $sql_find_new);
        ...
        while ($row_find_new = mysqli_fetch_object($result_find_new)) {
            ...
            if ($total_results > 0) {
                ...
                $sql_update_old = "UPDATE currencies
                                   ...
                                   WHERE id = '" . $row->id . "'";
                $result_update_old = mysqli_query($dbcon, $sql_update_old);

                $sql_delete_new = "DELETE FROM currencies
                                   WHERE id = '" . $row_find_new->id . "'";
                $result_delete_new = mysqli_query($dbcon, $sql_delete_new);
...
if ($current_db_version === '2.0037') {
    ...
    while ($row = mysqli_fetch_object($result)) {
        ...
        $sql_conversion = "SELECT id, conversion
                           FROM currencies
                           WHERE conversion != '0'";
        $result_conversion = mysqli_query($dbcon, $sql_conversion);
        ...
        while ($row_conversion = mysqli_fetch_object($result_conversion)) {
            ...
            $result_insert = mysqli_query($dbcon, $sql_insert);
