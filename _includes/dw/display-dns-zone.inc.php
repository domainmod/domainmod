    ...
    while ($row_dw_dns_zone_temp = mysqli_fetch_object($result_dw_dns_zone_temp)) {
        ...
        if ($from_main_dw_dns_zone_page == 1) { ?>
            ...
            if (mysqli_num_rows($result_temp) > 0 && $from_main_dw_dns_zone_page == 1) { ?>
                ...
                <BR><BR>[<a class="covert_link"
                            href="list-accounts.php?domain=<?php echo $row_dw_dns_zone_temp->domain; ?>">account</a>]<?php
        ...
        <td>
            ...
            <?php if ($from_main_dw_dns_zone_page == 0) { ?><font
                class="default_highlight"><?php echo $row_dw_dns_zone_temp->dw_server_name; ?></font><BR><BR><?php } ?>
            <table class="main_table_no_right_padding" cellpadding="0" cellspacing="0"><?php
                ...
                while ($row_get_records = mysqli_fetch_object($result_get_records)) { ?>
                    ...
                    <td class="main_table_cell_active_top_aligned_no_right_padding" width="70" align="right"
                        ...
                        <?php if ($row_get_records->type != "") { ?>
                            <strong><?php echo $row_get_records->type; ?></strong><?php } ?>
                    </td>
                    ...
                    <td class="main_table_cell_active_top_aligned_no_right_padding">
                        ...
                        <?php if ($row_get_records->address != "") { ?> | <?php if ($row_get_records->type != "A") { ?>
                            <strong>Address:</strong><?php } ?> <?php echo $row_get_records->address; ?><?php } ?>
                        ...
                        <?php if ($row_get_records->retry != "" && $row_get_records->retry != '0') { ?> |
                            <strong>Retry:</strong> <?php echo $row_get_records->retry; ?><?php } ?>
                        <?php if ($row_get_records->expire != "") { ?> |
                            <strong>Expire:</strong> <?php echo $row_get_records->expire; ?><?php } ?>
                        <?php if ($row_get_records->minimum != "" && $row_get_records->minimum != '0') { ?> | <strong>Minimum
                            TTL:</strong> <?php echo $row_get_records->minimum; ?><?php } ?>
                        <?php if ($row_get_records->cname != "") { ?> | <?php echo $row_get_records->cname; ?><?php } ?>
                        <?php if ($row_get_records->nsdname != "") { ?> | <?php if ($row_get_records->type != "NS") { ?>
                            <strong>NSDNAME:</strong> <?php } ?><?php echo $row_get_records->nsdname; ?><?php } ?>
