            ...
            <tr class="main_table_row_active_no_hover"><?php
				...
				if ($from_main_dw_dns_zone_page == 1) { ?>
                    ...
                    <td class="main_table_cell_active_top_aligned"><?php
                        ...
                        <strong><?=$visible_zonefile?></strong><?php
                        
                        if ($_SESSION['dw_view_all'] == "1") { ?>
                        
                            <BR><BR><?=$row_dw_dns_zone_temp->dw_server_name?><?php
                        ...
                        if (mysql_num_rows($result_temp) > 0 && $from_main_dw_dns_zone_page == 1) { ?>
                            ...
                            <BR><BR>[<a class="covert_link" href="list-accounts.php?domain=<?=$row_dw_dns_zone_temp->domain?>">account</a>]<?php
                ...
                <td>
                    ...
                    <?php if ($from_main_dw_dns_zone_page == 0) { ?><font class="default_highlight"><?=$row_dw_dns_zone_temp->dw_server_name?></font><BR><BR><?php } ?>
						...
						while ($row_get_records = mysql_fetch_object($result_get_records)) { ?>
							...
							<tr class="main_table_row_active_no_right_padding">
								<td class="main_table_cell_active_top_aligned_no_right_padding" width="70" align="right" valign="top">
									...
									<?php if ($row_get_records->type != "") { ?> <strong><?=$row_get_records->type?></strong><?php } ?>
								</td>
								<td class="main_table_cell_active_top_aligned_no_right_padding" width="46" align="center">
									| <?php if ($row_get_records->line < 10) echo "0"; ?><?=$row_get_records->line?> |
								</td>
								<td class="main_table_cell_active_top_aligned_no_right_padding">
									<?php if ($row_get_records->name != "") { ?><?php if ($row_get_records->type != "SOA" && $row_get_records->type != "NS" && $row_get_records->type != "A" && $row_get_records->type != "MX" && $row_get_records->type != "TXT" && $row_get_records->type != "CNAME") { ?>Name: <?php } ?><?=$row_get_records->name?><?php } ?>
									<?php if ($row_get_records->address != "") { ?> | <?php if ($row_get_records->type != "A") { ?><strong>Address:</strong><?php } ?> <?=$row_get_records->address?><?php } ?>
									<?php if ($row_get_records->exchange != "") { ?> | <?=$row_get_records->exchange?><?php } ?>
									<?php if ($row_get_records->preference != "" && $row_get_records->type == "MX") { ?> | <strong>Preference:</strong> <?=$row_get_records->preference?><?php } ?>
									<?php if ($row_get_records->mname != "") { ?> | <?=$row_get_records->mname?><?php } ?>
									<?php if ($row_get_records->rname != "") { ?> | <?=$row_get_records->rname?><?php } ?>
									<?php if ($row_get_records->type == "SOA") { ?><?php if ($row_get_records->ttl != "" && $row_get_records->ttl != '0') { ?> | <?php if ($row_get_records->type != "ZONE TTL" && $row_get_records->type != "SOA" && $row_get_records->type != "NS" && $row_get_records->type != "A" && $row_get_records->type != "MX" && $row_get_records->type != "TXT" && $row_get_records->type != "CNAME" && $row_get_records->type != ":RAW" && $row_get_records->type != "COMMENT") { ?>TTL: <?php } ?><?=$row_get_records->ttl?><?php } ?><?php } ?>
									<?php if ($row_get_records->serial != "" && $row_get_records->serial != '0') { ?><BR><strong>Serial:</strong> <?=$row_get_records->serial?><?php } ?>
									<?php if ($row_get_records->refresh != "" && $row_get_records->refresh != '0') { ?> | <strong>Refresh:</strong> <?=$row_get_records->refresh?><?php } ?>
									<?php if ($row_get_records->retry != "" && $row_get_records->retry != '0') { ?> | <strong>Retry:</strong> <?=$row_get_records->retry?><?php } ?>
									<?php if ($row_get_records->expire != "") { ?> | <strong>Expire:</strong> <?=$row_get_records->expire?><?php } ?>
									<?php if ($row_get_records->minimum != "" && $row_get_records->minimum != '0') { ?> | <strong>Minimum TTL:</strong> <?=$row_get_records->minimum?><?php } ?>
									<?php if ($row_get_records->cname != "") { ?> | <?=$row_get_records->cname?><?php } ?>
									<?php if ($row_get_records->nsdname != "") { ?> | <?php if ($row_get_records->type != "NS") { ?><strong>NSDNAME:</strong> <?php } ?><?=$row_get_records->nsdname?><?php } ?>
									<?php if ($row_get_records->raw != "") { ?><?=$wrapped_raw?><?php } ?>
									<?php if ($row_get_records->txtdata != "") { ?> | <?=$wrapped_txtdata?><?php } ?>
