...
?>
The below domains are in the segment <strong><font class="highlight"><?=$segment_name?></font></strong>, and they match domains that are stored in your <?=$software_title?> database, however the domains are currently marked as invactive.<BR><BR>
<a href="<?=$_SERVER['HTTP_REFERER']?>">&laquo; Back to Domains</a><BR><BR>
<a href="segments-inactive.php?segid=<?=$segid?>&export=1">Export Results</a><BR><BR>
<?php
while ($row = mysql_fetch_object($result)) {
	echo $row->domain . "<BR>";
}
