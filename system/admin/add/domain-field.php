...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $new_name != "" && $new_field_name != "" && CheckCustomFieldFormat($new_field_name)) {
	...
	if ($existing_field_name == 1) {
		...
		$sql = "INSERT INTO domain_fields 
				...
				('" . mysql_real_escape_string($new_name) . "', '" . $new_field_name . "', '" . mysql_real_escape_string($new_description) . "', '" . $new_field_type_id . "', '" . mysql_real_escape_string($new_notes) . "', '" . $current_timestamp . "')";
