...
require_once DIR_INC . '/settings/reporting-ssl-cost-by-month.inc.php';
...
$result = $pdo->query("
    SELECT sslc.id, YEAR(sslc.expiry_date) AS year, MONTH(sslc.expiry_date) AS month
        ...
        if ($result) {
            ...
            foreach ($result as $row) {
                ...
                $result_monthly_cost = $pdo->query("
                    SELECT SUM(sslc.total_cost * cc.conversion) AS monthly_cost
                    FROM ssl_certs AS sslc, ssl_fees AS f, currencies AS c, currency_conversions AS cc
                    WHERE sslc.fee_id = f.id
                      AND f.currency_id = c.id
                      AND c.id = cc.currency_id
                ...
                $result_yearly_cost = $pdo->query("
                    SELECT SUM(sslc.total_cost * cc.conversion) AS yearly_cost
                    FROM ssl_certs AS sslc, ssl_fees AS f, currencies AS c, currency_conversions AS cc
                    WHERE sslc.fee_id = f.id
                      AND f.currency_id = c.id
                      AND c.id = cc.currency_id
