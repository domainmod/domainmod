<?php
// /reporting/ssl/cost-by-category.php
// 
...
$page_subtitle = "SSL Certificate Cost by Category Report";
$software_section = "reporting-ssl-cost-by-category-report";
$report_name = "ssl-cost-by-category-report";
...
$sql = "SELECT cat.id, cat.name, SUM(f.renewal_fee * cc.conversion) as total_cost, count(*) AS number_of_certs
...
if ($submission_failed != "1" && $total_rows > 0) {
	...
	if ($export == "1") {
		...
		if ($all == "1") {
			$export_filename = "ssl_cost_by_category_report_all_" . $current_timestamp_unix . ".csv";
		} else {
			$export_filename = "ssl_cost_by_category_report_" . $new_start_date . "--" . $new_end_date . ".csv";
		}
...
if ($submission_failed != "1" && $total_rows > 0) { ?>
	...
	while ($row = mysql_fetch_object($result)) {
		...
		<tr class="main_table_row_active">
			<td class="main_table_cell_active"><a class="invisiblelink" href="../../ssl-certs.php?sslpcid=<?=$row->id?>"><?=$row->name?></a></td>
			<td class="main_table_cell_active"><a class="invisiblelink" href="../../ssl-certs.php?sslpcid=<?=$row->id?>"><?=$row->number_of_certs?></a></td>
