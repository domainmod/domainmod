...
if ($submission_failed != "1" && $total_rows > 0) {
    ...
    if ($export_data == "1") {
        ...
        if ($all == "1") {
            ...
            $export_file = $export->openFile('ssl_cost_by_category_report_all', strtotime($time->time()));

        } else {

            $export_file = $export->openFile(
                'ssl_cost_by_category_report',
...
<form name="export_ssl_form" method="post">
    ...
    <?php if ($total_rows > 0) { ?>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>[<a
                href="cost-by-category.php?export_data=1&new_start_date=<?php echo $new_start_date; ?>&new_end_date=<?php echo $new_end_date; ?>&all=<?php echo $all; ?>">EXPORT
                REPORT</a>]</strong>
    <?php } ?>
</form>
...
if ($submission_failed != "1" && $total_rows > 0) { ?>
    ...
    while ($row = mysqli_fetch_object($result)) {
        ...
        <td class="main_table_cell_active"><a class="invisiblelink"
                                              href="../../ssl-certs.php?sslpcid=<?php echo $row->id; ?>"><?php echo $row->name; ?></a>
        </td>
        <td class="main_table_cell_active"><a class="invisiblelink"
                                              href="../../ssl-certs.php?sslpcid=<?php echo $row->id; ?>"><?php echo $row->number_of_certs; ?></a>
        </td>
