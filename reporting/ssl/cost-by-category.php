...
$result = $pdo->query("
    SELECT cat.id, cat.name, SUM(sslc.total_cost * cc.conversion) AS total_cost, count(*) AS number_of_certs
...
if ($submission_failed != '1' && $total_rows > 0) {
    ...
    if ($export_data === 1) {
        ...
        if ($daterange == '') {
            ...
            $export_file = $export->openFile(_('ssl_cost_by_category_report_all'), strtotime($time->stamp()));

        } else {

            $export_file = $export->openFile(
                _('ssl_cost_by_category_report'),
