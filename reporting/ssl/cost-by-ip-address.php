...
$sql = "SELECT ip.id, ip.name, ip.ip, ip.rdns, SUM(f.renewal_fee * cc.conversion) as total_cost, count(*) AS number_of_certs
...
if ($submission_failed != "1" && $total_rows > 0) {
	...
	if ($export == "1") {
		...
		$full_export .= "\"IP Address Name\",\"IP Address\",\"rDNS\",\"SSL Certs\",\"Cost\",\"Per Cert\"\n";
		...
		while ($row = mysql_fetch_object($result)) {
			...
			$full_export .= "\"" . $row->name . "\",\"" . $row->ip . "\",\"" . $row->rdns . "\",\"" . $row->number_of_certs . "\",\"" . $row->total_cost . "\",\"" . $per_cert . "\"\n";
...
if ($submission_failed != "1" && $total_rows > 0) { ?>
	...
	while ($row = mysql_fetch_object($result)) {
		...
		<tr class="main_table_row_active">
			<td class="main_table_cell_active"><a class="invisiblelink" href="../../ssl-certs.php?sslipid=<?=$row->id?>"><?=$row->name?></a></td>
			<td class="main_table_cell_active"><a class="invisiblelink" href="../../ssl-certs.php?sslipid=<?=$row->id?>"><?=$row->ip?></a></td>
			<td class="main_table_cell_active"><a class="invisiblelink" href="../../ssl-certs.php?sslipid=<?=$row->id?>"><?=$row->rdns?></a></td>
			<td class="main_table_cell_active"><a class="invisiblelink" href="../../ssl-certs.php?sslipid=<?=$row->id?>"><?=$row->number_of_certs?></a></td>
