...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($new_start_date > $new_end_date) {
        ...
        if ($new_start_date > $new_end_date) $_SESSION['s_message_danger'] .= _('The end date proceeds the start date') . '<BR>';
...
$result = $pdo->query("
    SELECT cat.id, cat.name, SUM(d.total_cost * cc.conversion) AS total_cost, count(*) AS number_of_domains
...
if ($submission_failed != '1' && $total_rows > 0) {
    ...
    if ($export_data === 1) {
        ...
        if ($daterange == '') {
            ...
            $export_file = $export->openFile(_('domain_cost_by_category_report_all'), strtotime($time->stamp()));

        } else {

            $export_file = $export->openFile(
                _('domain_cost_by_category_report'),
    ...
    <table id="<?php echo $slug; ?>" class="<?php echo $datatable_class; ?>">
        ...
        foreach ($result as $row) {
            ...
            <tr>
                ...
                <td><a href="../../domains/index.php?pcid=<?php echo $row->id; ?>"><?php echo $row->number_of_domains; ?></a></td>
