...
$sql = "SELECT o.id, o.name, SUM(f.renewal_fee * cc.conversion) as total_cost, count(*) AS number_of_domains
...
if ($submission_failed != "1" && $total_rows > 0) {
	...
	if ($export == "1") {
		...
		$full_export .= "\"Owner\",\"Domains\",\"Cost\",\"Per Domain\"\n";
		...
		if ($all == "1") {
			$export_filename = "domain_cost_by_owner_report_all_" . $current_timestamp_unix . ".csv";
		} else {
			$export_filename = "domain_cost_by_owner_report_" . $new_start_date . "--" . $new_end_date . ".csv";
		}
...
if ($submission_failed != "1" && $total_rows > 0) { ?>
	...
	while ($row = mysql_fetch_object($result)) {
		...
		<tr class="main_table_row_active">
			<td class="main_table_cell_active"><a class="invisiblelink" href="../../domains.php?oid=<?=$row->id?>"><?=$row->name?></a></td>
			<td class="main_table_cell_active"><a class="invisiblelink" href="../../domains.php?oid=<?=$row->id?>"><?=$row->number_of_domains?></a></td>
